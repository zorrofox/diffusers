...
transformer_shardings = {
    ...
    # (torch.Size([8960, 1536]), torch.bfloat16)
    r'blocks.\d+.ffn.net.0.proj.weight': (axis,),
    # (torch.Size([8960]), torch.bfloat16)
...
text_encoder_shardings = {
    # (torch.Size([256384, 4096]), torch.bfloat16)
    'shared.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([4096, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.SelfAttention.q.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([4096, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.SelfAttention.k.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([4096, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.SelfAttention.v.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([4096, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.SelfAttention.o.weight': (None, (axis, 'dp', 'sp')),
    ...
    # (torch.Size([10240, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.DenseReluDense.wi_0.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([10240, 4096]), torch.bfloat16)
    'encoder.block.*.layer.*.DenseReluDense.wi_1.weight': ((axis, 'dp', 'sp'), ),
    # (torch.Size([4096, 10240]), torch.bfloat16)
    'encoder.block.*.layer.*.DenseReluDense.wo.weight': (None, (axis, 'dp', 'sp')),
...
def scaled_dot_product_attention(
    ...
    # VAE typically has larger hidden dimensions (384)
    if query.shape[-1] >= 385:
    ...
    if env.config.use_tpu_splash_attention:
        ...
        res = _tpu_splash_attention(
            jquery, jkey, jvalue, env, scale=scale, is_causal=is_causal, window_size=window_size)
        ...
        if ckpt_path is not None:
            ...
            for pt_key, tensor in tensors.items():
                ...
                if "encoder" in renamed_pt_key:
                    renamed_pt_key = renamed_pt_key.replace(
                        "resample.1", "resample.conv")
                pt_tuple_key = tuple(renamed_pt_key.split("."))
                flax_key, flax_tensor = rename_key_and_reshape_tensor(
                    pt_tuple_key, tensor, eval_shapes)
                flax_key = tuple(int(item) if isinstance(
                    item, str) and item.isdigit() else item for item in flax_key)

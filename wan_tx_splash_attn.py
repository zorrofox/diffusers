# =========================================================================
# START: Corrected CustomWanPipeline (v2)
# =========================================================================
...
class CustomWanPipeline(WanPipeline):
    # The __call__ method remains the same as the last working version.
    @torch.no_grad()
    def __call__(
        self,
        prompt,
        ...
        guidance_scale,
        generator=None,
        output_type="np",  # 官方默认是"np"
        use_dp=False,
        enable_teacache=False,
        teacache_thresh=0.2,
        use_ret_steps=False,
    ):
        # TeaCache state initialization
        if not hasattr(self, "ret_steps"):
            self.ret_steps = 1
        if not hasattr(self, "cutoff_steps"):
            self.cutoff_steps = 1000
        if not hasattr(self, "coefficients"):
            self.coefficients = np.poly1d([1.0, 0.0])
        ...
        # 1. 计算VAE缩放因子
        vae_scale_factor_temporal = 2 ** sum(self.vae.temperal_downsample) if hasattr(self.vae, "temperal_downsample") else 4
        vae_scale_factor_spatial = 2 ** len(self.vae.temperal_downsample) if hasattr(self.vae, "temperal_downsample") else 8
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            # 修复：根据use_dp参数决定如何处理classifier-free guidance
            if use_dp:
                # 使用stacked approach，与原始WanPipeline的use_dp=True逻辑一致
                latent_model_input_stacked = torch.cat([latents, latents]).to(self.transformer.dtype)
                timestep = t.expand(latents.shape[0] * 2 if guidance_scale > 1.0 else 1)
                ...
                if enable_teacache:
                    ...
                    # --- Start of Corrected TeaCache Logic ---
                    # 1. 为 conditioned prediction 决定是否计算
                    should_calc_cond = True
                    if self.cnt >= self.ret_steps and self.cnt < self.cutoff_steps and self.previous_e0_even is not None:
                        rel_l1_distance = ((latent_model_input - self.previous_e0_even).abs().mean() / self.previous_e0_even.abs().mean()).cpu().item()
                        rescale_func = np.poly1d(self.coefficients)
                        self.accumulated_rel_l1_distance_even += rescale_func(rel_l1_distance)
                        ...
                        self.accumulated_rel_l1_distance_even = 0
                    self.previous_e0_even = latent_model_input.clone()

                    # 2. 获取 conditioned prediction
                    if not should_calc_cond and self.previous_residual_even is not None:
                        noise_pred_cond = self.previous_residual_even
                    else:
                        ...
                        self.previous_residual_even = noise_pred_cond.clone()

                    if guidance_scale > 1.0:
                        # 3. 为 unconditioned prediction 决定是否计算
                        should_calc_uncond = True
                        if self.cnt >= self.ret_steps and self.cnt < self.cutoff_steps and self.previous_e0_odd is not None:
                            rel_l1_distance = ((latent_model_input - self.previous_e0_odd).abs().mean() / self.previous_e0_odd.abs().mean()).cpu().item()
                            rescale_func = np.poly1d(self.coefficients)
                            self.accumulated_rel_l1_distance_odd += rescale_func(rel_l1_distance)
                            ...
                            self.accumulated_rel_l1_distance_odd = 0
                        self.previous_e0_odd = latent_model_input.clone()

                        # 4. 获取 unconditioned prediction
                        if not should_calc_uncond and self.previous_residual_odd is not None:
                            noise_pred_uncond = self.previous_residual_odd
                        else:
                            ...
                            self.previous_residual_odd = noise_pred_uncond.clone()
                    ...
                    self.cnt += 1
                    # --- End of Corrected TeaCache Logic ---
                else:
                    # 不使用TeaCache，正常计算
                    stacked_noise = self.transformer(
                        hidden_states=latent_model_input_stacked,
                        timestep=timestep,
                        encoder_hidden_states=encoder_hidden_state,
                        return_dict=False,
                    )[0]
                    # 分离conditioned和unconditioned的预测
                    noise_pred_uncond, noise_pred_cond = stacked_noise.chunk(2)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
            else:
                # 使用分别计算approach，与原始WanPipeline的use_dp=False逻辑一致
                latent_model_input = latents.to(self.transformer.dtype)
                timestep = t.expand(latents.shape[0])
        ...
        if output_type == "latent":
            return LatentOutput(latents)

        #print("before VAE :", latents.shape) #(1, 16, 6, 90, 160)=>(1, 16, 21, 90, 160)
        ...
        if video.shape[-1] == 3 and video.shape[0] == 1 and video.ndim == 5:
            # (1, T, H, W, 3) -> (T, H, W, 3)
            video = video[0]
        elif video.shape[1] == 3 and video.ndim == 4:
            # (C, T, H, W) -> (T, H, W, C)
    ...
    # We only modify this internal _encode_prompt method
    def _encode_prompt(
        self,
        prompt,
        device,
        ...
        if prompt_embeds is None:
            ...
            # Replace self.tokenizer.model_max_length with a safe, standard value.
            text_inputs = self.tokenizer(
                prompt,
                padding="max_length",
                max_length=77, # Use a fixed, safe value
                truncation=True,
                return_tensors="pt",
            )
        ...
        if self.text_encoder is not None:
            if isinstance(self, WanPipeline):
                prompt_embeds = prompt_embeds
            else:
                prompt_embeds = prompt_embeds.to(dtype=self.text_encoder.dtype, device=device)

        bs_embed, seq_len, _ = prompt_embeds.shape
        if bs_embed != batch_size:
        ...
        if do_classifier_free_guidance:
            if negative_prompt_embeds is None:
                ...
                if batch_size != len(negative_prompt):
                    raise ValueError(f"Batch size mismatch for negative_prompt.")
                
                # ----> THIS IS THE ONLY OTHER CHANGE <----
                uncond_tokens = self.tokenizer(
                    negative_prompt,
                    padding="max_length",
                    max_length=77, # Use a fixed, safe value here as well
                    truncation=True,
                    return_tensors="pt",
                ).input_ids.to(device)
                negative_prompt_embeds = self.text_encoder(uncond_tokens)[0]
...
# =========================================================================
# END: New Integration Strategy with CustomWanPipeline
# =========================================================================
...
def main():
  ...
  try:
    ...
    # ----> 使用新的 CustomWanPipeline <----
    pipe = CustomWanPipeline.from_pretrained(model_id, torch_dtype=torch.bfloat16, use_safetensors=True)
